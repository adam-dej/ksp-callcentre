...
import sk.ksp.callcentrum.incall.InCallControlState;
...
//    private static final boolean VDBG = (PhoneGlobals.DBG_LEVEL >= 2);
     ...
     * initially visible when bringing up the InCallActivity.  (If this
    ...
    // false.
    public static final String ACTION_UNDEFINED = "com.android.phone.InCallActivity.UNDEFINED";
...
//    private ManageConferenceUtils mManageConferenceUtils;
    ...
    // to determine what should happen when exiting the InCallActivity after a
...
//            // There should be no way to ever reach the InCallActivity on a
...
//        Profiler.callScreenCreated();
    ...
    /**
     * Sets the Phone object used internally by the InCallActivity.
     *
    ...
    protected void onResume() {
        ...
        //
        // When the InCallActivity comes to the foreground, we normally we
        ...
        if (inCallUiState.hasPendingCallStatusCode()) {
            ...
            // (We need to stay here in the InCallActivity so that the user
        ...
        // UI; it's totally useless when *entering* the InCallActivity.)
        ...
        if (status != SyncWithPhoneStateStatus.SUCCESS) {
            ...
            if (handledStartupError) {
                ...
                // the InCallActivity to come back to the foreground.)
                //
                // In this scenario we do NOT want to stay here on the
                // InCallActivity: we're not showing any useful info to the
                ...
                // Force the InCallActivity to truly finish(), rather than just
                ...
                // InCallActivity can get stuck in an inconsistent state, somehow
        ...
        Profiler.profileViewCreate(getWindow(), com.android.phone.InCallActivity.class.getName());
    ...
    protected void onPause() {
        ...
        // of the InCallActivity, so we only care about them while we're in the
        // foreground.)
    ...
    protected void onDestroy() {
        ...
        // Clear out the InCallActivity references in various helper objects
     ...
     *
     * This must be called when an InCallActivity session has
     ...
     *
     * @param forceFinish If true, force the InCallActivity to
    ...
    /* package */ void updateAfterRadioTechnologyChange() {
        ...
        // causing us to exit the InCallActivity.)
        requestUpdateScreen();
    }
    ...
    private void internalResolveIntent(Intent intent) {
        ...
        // In gingerbread and earlier releases, the InCallActivity used to
        ...
        //
        // But it doesn't make sense to tie those actions to the InCallActivity
        // (or especially to the *activity lifecycle* of the InCallActivity).
        // Instead, the InCallActivity should only be concerned with running the
        ...
        //
        // So now, the InCallActivity is only ever launched using the ACTION_MAIN
        ...
        if (action.equals(OtaUtils.ACTION_PERFORM_CDMA_PROVISIONING)) {
            ...
            throw new IllegalStateException(
                "Unexpected ACTION_PERFORM_CDMA_PROVISIONING received by InCallActivity: "
                + intent);
            ...
            // launching the InCallActivity directly.
            throw new IllegalStateException("Unexpected CALL action received by InCallActivity: "
                                            + intent);
    ...
    private boolean handleCallKey() {
        ...
        if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
            ...
            if (hasRingingCall) {
                ...
                // sure that we'll respond to the key even if the InCallActivity
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_CALL:
                ...
                if (!handled) {
                    Log.w(LOG_TAG, "InCallActivity should always handle KEYCODE_CALL in onKeyDown");
                }
                // Always consume CALL to be sure the PhoneWindow won't do anything with it
                return true;
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE:
                if (mCM.getState() == PhoneConstants.State.RINGING) {
                    ...
                    // the InCallActivity hasn't come to the foreground yet.)
                    //
                    // We'd only ever get here in the extremely rare case that the
                    // incoming call started ringing *after*
                    // PhoneWindowManager.interceptKeyTq() but before the event
            ...
            case KeyEvent.KEYCODE_EQUALS:
                if (VDBG) {
                    log("----------- InCallActivity call state dump --------------");
...
//    }
    ...
    private void onDisconnect(AsyncResult r) {
        ...
        // (This might affect what happens after we exit the InCallActivity; see
        // delayedCleanupAfterDisconnect().)
        ...
        if (bailOutImmediately && showingQuickResponseDialog) {
            ...
            // call connection hence we should *not* bring down the InCallActivity as the Phone
            ...
            if (mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                if (!currentlyIdle) {
                    ...
                    if (DBG) log("onDisconnect: Call Collision case - staying on InCallActivity.");
            ...
            //
            // TODO: Consider moving this to CallNotifier. This code assumes the InCallActivity
            // never gets destroyed. For this exact case, it works (since InCallActivity won't be
            ...
            if (currentlyIdle && !isForegroundActivity() && isForegroundActivityForProximity()) {
                ...
                // This variable will be kept true until the next InCallActivity#onPause(), which
                ...
                // InCallActivity#onResume() will set DELAYED_CLEANUP_AFTER_DISCONNECT message,
    ...
    private void onMMICancel() {
        ...
        if (DBG) log("onMMICancel: finishing InCallActivity...");
        dismissAllDialogs();
        endInCallScreenSession();
    }
    ...
    private void onMMIComplete(MmiCode mmiCode) {
        ...
        if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
            ...
            if (mmiCode.getState() != MmiCode.State.PENDING) {
                if (DBG) log("Got MMI_COMPLETE, finishing InCallActivity...");
                dismissAllDialogs();
                endInCallScreenSession();
            }
        }
     ...
     * "dialable" key events here in the InCallActivity: we do directly to the
    ...
    private void updateScreen() {
        ...
        if (mCM.getState() == PhoneConstants.State.RINGING) {
            ...
            // the InCallActivity doesn't keep enough state right now to notice
    ...
    private SyncWithPhoneStateStatus syncWithPhoneState() {
        ...
        // Finally, it's also OK to stay here on the InCallActivity if we
    ...
    private void addVoiceMailNumberPanel() {
        ...
        if (DBG) log("addVoiceMailNumberPanel: finishing InCallActivity...");
        endInCallScreenSession();
    ...
    private void dontAddVoiceMailNumber() {
        ...
        if (DBG) log("dontAddVoiceMailNumber: finishing InCallActivity...");
        endInCallScreenSession();
    }
     ...
     * nothing, and instead stay here on the InCallActivity.
     */
    private void delayedCleanupAfterDisconnect() {
        ...
        // There are two cases where we should *not* exit the InCallActivity:
        ...
        if (stayHere) {
            if (DBG) log("- delayedCleanupAfterDisconnect: staying on the InCallActivity...");
        } else {
            ...
            if (mIsForegroundActivity) {
                if (DBG) log("- delayedCleanupAfterDisconnect: finishing InCallActivity...");
    ...
    private void showStatusIndication(CallStatusCode status) {
        switch (status) {
            case SUCCESS:
                // The InCallActivity does not need to display any kind of error indication,
            ...
            case CDMA_CALL_LOST:
                // This status indicates that InCallActivity should display the
            ...
            case EXITED_ECM:
                // This status indicates that InCallActivity needs to display a
    ...
    private void showCallLostDialog() {
        ...
        // Don't need to show the dialog if InCallActivity isn't in the forgeround
        if (!mIsForegroundActivity) {
    ...
    private void showExitingECMDialog() {
        ...
        // dialog), but do *not* bail out of the InCallActivity.
    ...
    private void bailOutAfterErrorDialog() {
        ...
        if (DBG) log("bailOutAfterErrorDialog(): end InCallActivity session...");
        ...
        // by the InCallActivity being paused or finished, since the resulting
     ...
     * by the InCallActivity.  Useful if (a) we're about to bring up
    ...
    private void internalHangup() {
        ...
        if (state == PhoneConstants.State.IDLE) {
            ...
            //   the InCallActivity with the phone truly idle, perhaps due to a
    ...
    /**
     * InCallActivity-specific wrapper around PhoneUtils.switchHoldingAndActive().
     */
    ...
    private void setInCallScreenMode(InCallScreenMode newMode) {
        ...
        switch (newMode) {
            ...
            case UNDEFINED:
                ...
                //  5) The screen will be blank (Should be normal InCallActivity)
    ...
    /**
     * Posts a handler message telling the InCallActivity to refresh the
     * onscreen in-call UI.
     *
     * This is just a wrapper around updateScreen(), for use by the
     * rest of the phone app or from a thread other than the UI thread.
     *
     * updateScreen() is a no-op if the InCallActivity is not the foreground
     * activity, so it's safe to call this whether or not the InCallActivity
    ...
    public boolean isPhoneStateRestricted() {
        ...
        // phone call is being made, to allow for input into the InCallActivity.
        // Having the InCallActivity judge the state of the device from this flag
    ...
    /**
     * Posts a handler message telling the InCallActivity to close
     ...
     * and InCallActivity is in pause mode.  CallNotifier will listen for call
    ...
    private boolean checkOtaspStateOnResume() {
        ...
        if (otaspUiActive) {
            // Make sure the OtaUtils instance knows about the InCallActivity's
            // OTASP-related UI widgets.
            //
            // (This call has no effect if the UI widgets have already been set up.
            // It only really matters  the very first time that the InCallActivity instance
...
//    }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        // Nothing else to do here, since (currently) the InCallActivity looks
     ...
     *
     * Watch out: this won't be called if InCallActivity isn't ready yet,
    ...
    private void onNewRingingConnection() {
        ...
        // where a call-waiting call comes in while the InCallActivity is
     ...
     */
    public void requestRemoveProviderInfoWithDelay() {
